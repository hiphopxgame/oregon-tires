<?php
/**
 * Oregon Tires — Bilingual Login Template (EN/ES)
 *
 * Local override of member-kit's login.php with translation support.
 * Uses memberT() via t() for all user-facing strings.
 *
...
<div class="member-page">
    <div class="member-card">
        ...
        <div class="member-header">
            <h1><?= htmlspecialchars(t('sign_in') ?? 'Sign In') ?></h1>
            <p><?= htmlspecialchars(t('welcome_back_sub') ?? 'Welcome back') ?></p>
        </div>
        ...
        <div id="session-timeout-warning" class="member-alert member-alert--warning"
            ...
            <?= htmlspecialchars(t('session_expiring') ?? 'Your session will expire in') ?> <span id="countdown">5:00</span>.
        ...
        <?php if ($verified): ?>
            <div class="member-alert member-alert--success">
                <?= htmlspecialchars(t('email_verified_success') ?? 'Email verified successfully. You can now sign in.') ?>
            </div>
        <?php endif; ?>

        <?php if ($registered): ?>
            <div class="member-alert member-alert--info">
                <?= htmlspecialchars(t('account_created_check_email') ?? 'Account created! Please check your email to verify your address before signing in.') ?><?= $maskedEmail ? " (<strong>{$maskedEmail}</strong>)" : '' ?>
            </div>
        <?php endif; ?>
        ...
        <form class="member-form" data-action="/api/member/login.php" data-method="POST"
            ...
            <div class="member-field">
                <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.25rem;">
                    <label class="member-label" for="login-email" style="margin-bottom:0;"><?= htmlspecialchars(t('email_label') ?? 'Email') ?></label>
            ...
            <button type="submit" class="member-btn" data-track="email-submit"><?= htmlspecialchars(t('sign_in_btn') ?? 'Sign In') ?></button>

            <div class="member-trust-badge" aria-label="Security information">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;">
                    <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
                </svg>
                <span><?= htmlspecialchars(t('encrypted_badge') ?? '256-bit encrypted · Your data stays private') ?></span>
            </div>
        </form>
        ...
        <div class="member-group" data-group="social">
            <div class="member-group-label"><?= htmlspecialchars(t('or_continue_with') ?? 'or continue with') ?></div>
        ...
        <div class="member-group" data-group="wallets">
            <div class="member-group-label"><?= htmlspecialchars(t('or_connect_wallet') ?? 'or connect wallet') ?></div>
...
<div id="rate-limit-message" class="member-alert member-alert--rate-limit" role="alert" aria-live="polite" style="display:none;">
    <strong><?= htmlspecialchars(t('too_many_requests') ?? 'Too many requests.') ?></strong>
    <span id="rate-limit-text"><?= htmlspecialchars(t('try_again_later') ?? 'Please try again later.') ?></span>
