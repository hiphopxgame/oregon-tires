-- Phase 1: Fix Critical RLS Security Issues (Updated)
-- Drop existing overly permissive policies that need to be replaced
...
-- Update gallery images policy (keep existing public read, add admin write)
DROP POLICY IF EXISTS "Public can view active gallery images" ON oretir_gallery_images;
...
-- Update service images policies
DROP POLICY IF EXISTS "Public can view active service images" ON oretir_service_images;
