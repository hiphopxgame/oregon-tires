...
INSERT INTO oretir_site_settings (setting_key, value_en, value_es) VALUES
  ...
  ('email_tpl_booking_body',     'We''ve received your appointment request for <strong>{{service}}</strong> on <strong>{{date}}</strong> at <strong>{{time}}</strong>.{{vehicle_line}}<br><br>We will contact you shortly to confirm your appointment.', 'Hemos recibido su solicitud de cita para <strong>{{service}}</strong> el <strong>{{date}}</strong> a las <strong>{{time}}</strong>.{{vehicle_line}}<br><br>Nos comunicaremos con usted pronto para confirmar su cita.'),
